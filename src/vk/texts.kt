...
const val adminAnswering =
    "Ты правда хочешь промокод? Есть у меня один пожизненный. С ним вообще всё везде бесплатно будет: IMNOTGAYBUT$20IS$20"
...
const val questionIdWasNotRecognized = "Не понял id вопроса, там число должно быть, а не вот это вот."
const val questionHasBeenUpdated = "Готово, босс, вопрос обновлён."
...
const val nobodyAnsweredCorrectly = "Все тупые. Никто не знает ответа на этот вопрос."
...
- "/править 12%вопрос%ответ" - заменю текст вопроса и ответ, на указанные, в вопросе с id 12. Ты можешь не писать часть, которую не нужно править, просто ничего не пиши. Главное - ставь знак %.;  
...
Если нет - команды придётся вводить вручную. Их список будет ниже.
...
const val rules = """
На ответ тебе даётся 30 секунд. Если твой ответ покажется мне неточным (опечатки), я могу переспросить. Так что, печатай аккуратно ;).
...
fun activeQuestionFormatted(q: Question): String {
    ...
    return """
        ...
        Добавлен: ${tsAsDateString(q.addedAt)}
    """.trimIndent()
}

fun questionFormatted(q: Question): String {
    return "${q.id}: ${q.text} - ${q.answer}. Код: ${q.promocode}. Добавлен: ${tsAsDateString(q.addedAt)}"
}
